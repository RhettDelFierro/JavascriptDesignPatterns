...
//variations of this include event handling, piping through a process and event source replay.
class ShoppingCart{
    constructor(){
        this.items = []
        //checkingOut
...
class CanCheckoutValidator extends ShoppingCartValidator{
    validate(cart){
        cart.isValid = false
        if (cart.status !== 'checkoudOut') cart.isValid = true
...
otherCart.items.push([{type: 'socks'}])
otherCart.status = 'checkoutOut'
otherCart.validate()
